            ...
            <ul class="wc_payment_methods payment_methods methods">
                ...
                if (!empty($available_gateways)) {
                    if ($order->get_payment_method()) {
                        $payment_method = $available_gateways[$order->get_payment_method()];
                        if(get_post_meta($order->get_id(), 'order_type', true) == "HOMETRIAL"){
                            // this doesn't do anything since the charge type is taken from the plugins settings in a later step
                            $payment_method->settings['charge_type'] = "authorize";
                        }
                        // var_dump($payment_method->payment_object);
                        wc_get_template('checkout/payment-method.php', array('gateway' => $payment_method));
                    } else {
                        foreach ($available_gateways as $gateway) {
                            // if statement to not show stripe_cc on normal checkout page bcs the charge_type is set to authorize
