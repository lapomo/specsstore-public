    ...
    if( 'on' !== WishSuite.option_data['btn_limit_login_off'] ){
        $body.on('click', 'a.wishsuite-btn', function (e) {
            ...
            var $this = $(this),
                ...
                currentText = $this.children('.wishsuite-btn-text').text();
            ...
            $.ajax({
                ...
                success: function ( response ) {
                    if ( response ) {
                        ...
                        $this.addClass('wishsuite-btn-exist');
                        $this.children('.wishsuite-btn-text').text( switchText );
        ...
        // remove shop product from wishlist
        $body.on('click', 'a.wishsuite-btn-exist', function (e) {
            e.preventDefault();
            var $this = $(this),
                id = $this.data('product_id'),
                switchText = $this.data('switch-text'),
            ...
            $.ajax({
                ...
                success: function ( response ) {
                    if ( response ) {
                        $this.removeClass('wishsuite-btn-exist');
                        $this.removeClass('loading');
                        $this.addClass('wishsuite-btn');
                        $this.children('.wishsuite-btn-text').text( switchText );
                        $this.data('switch-text', currentText);
                        $body.find('.wishsuite-counter').html( response.data.item_count );
                    } else {
